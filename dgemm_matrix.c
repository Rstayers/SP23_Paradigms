...
void dgemm_parallel_no_blocking( double** a, double** b, double** c, int N, int num_threads ){
	...
	#pragma omp parallel shared(a, b, c)
	{
		...
		#pragma omp for private(iter, jter, kter) schedule(static, 32768)
		for( iter = 0; iter < (int)N; ++iter ){
...
void dgemm_parallel_blocking( double** a, double** b, double** c, int N, int num_threads ){
	...
	#pragma omp parallel shared(a, b, c)
	{
		...
		int n_per_thread = N / num_threads;  // Dividing work among threads
		int start_index = tid * n_per_thread;
		int end_index = (tid + 1) * n_per_thread - 1;
		if (tid == num_threads - 1) {
				end_index = N - 1;  // Handling the last chunk
		}
